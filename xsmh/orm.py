...
class ModelMetaclass(type):
	def __new__(cls,name,bases,attrs):
		...
		attrs['__update__'] = 'update `%s` set %s where `%s` = ?'%(tableName,', '.join(map(lambda x:x+'=?',escaped_fields)),primary_key)
...
class Model(dict,metaclass=ModelMetaclass):
	...
	@classmethod		
	async def update(cls,primary_key=None,upd={}):
		if primary_key is None:
			raise ValueError('primary_key must exist and cannot be None')
		args = []	
		sql = []
		...
		rows = await execute('update `%s` set %s where `%s` = ?'%(cls.__table__,', '.join(map(lambda x:x+'=?',sql)),cls.__primary_key__),args)      
		if rows != 1:
			logging.warn('failed to update by primary key: affected rows: %s' % rows)
			
	@classmethod				    	        
	async def remove(cls,primary_key=None):
		if primary_key is None:
			raise ValueError('primary_key must exist and cannot be None')		
		args = [primary_key]
...
# print(loop.run_until_complete(execute(sql,('change',5))))
# sql = 'select * from test '
# print(loop.run_until_complete(select(sql,())))
# loop.run_forever()